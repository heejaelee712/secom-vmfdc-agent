# secom-vm-fdc-agent
SECOM 반도체 센서 데이터로 불량을 사전 예측하고, 비용 최적화 기반 선별검사 의사결정을 지원하는 ML 파이프라인 + SHAP/LLM 가이던스 에이전트

# SECOM 반도체 불량 예측 및 VM-FDC 가이던스 에이전트

> UCI SECOM 센서 데이터를 활용해 웨이퍼 불량을 사전 예측하고, 비용 기준에 따라 선별검사를 자동화하는 ML 파이프라인과 XAI/LLM 기반 진단 에이전트를 구현한 개인 포트폴리오 프로젝트입니다.
...
단순히 모델 성능 지표(Accuracy, AUC)를 올리는 것을 목표로 하지 않고, **데이터 증강 기법이 만들어내는 인공적 신호(Artifact)를 검증하고, 비용 구조를 고려한 의사결정까지 이어지는 전체 파이프라인**을 구축하는 데 중점을 두었습니다.
